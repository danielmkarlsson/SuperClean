CleanRemoteSoundfileInfo {
	...
	*new { |library, port|
	...
	start { |port (57120) |
		...
		}, "/clean_soundfileinfo_request", recvPort: port).fix;
	}
	...
	sendRemoteSoundFileInfoReply { |netAddr|
		...
		netAddr.sendMsg("/clean_soundfileinfo_reply", *info);
	}
	...
	convertBuffersToInfo { |library|
		...
		library.bufferEvents.keysValuesDo { |key, eventArray|
			eventArray.do { |event|
				...
				if(bufnum.notNil) {
					info = info.add(key);
					info = info.add(event[\bufNumFrames]);
	...
	convertInfoToBuffers { |info|
		...
		info.clump(4).do { |data|
			var key, bufNumFrames, bufNumChannels, bufnum, buf;
			# key, bufNumFrames, bufNumChannels, bufnum = data;
			buf = Buffer(nil, bufNumFrames, bufNumChannels, bufnum);
	...
	sendRequest { |addr, callback, timeout = 5|
		...
		}, "/clean_soundfileinfo_reply", recvPort: addr.port);

		addr.sendMsg("/clean_soundfileinfo_request");
		fork {
			timeout.wait;
			resp.free;
		}
