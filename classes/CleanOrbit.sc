...
CleanOrbit {
	...
	*new { |clean, outBus, orbitIndex = 0|
		^super.newCopyArgs(clean, outBus, orbitIndex).init
	}

	init {
		server = clean.server;
		if(server.serverRunning.not) {
			Error("SuperColldier server '%' not running. Couldn't start CleanOrbit".format(server.name)).warn;
			^this
		};
